...
const Number: FC<NumberProps> = ({ number, timeUnit }) => {
  ...
  return (
    <div className={"w-[64px] h-[64px] flex justify-center font-bebasNeue"}>
      <div className={"w-[49px] h-[60px] text-center text-huge leading-[103%] relative"}>
        <div className={isLittleTime ? "text-FD4D35" : "text-01B9C5100"}>{number.toString().padStart(2, "0")}</div>
        <div
          className={
            "font-ptRootUIBold text-[16px] text-818798 leading-[130%] absolute left-[50%] bottom-[-11px] translate-x-[-50%]"
          }
        >
          {timeUnit}
        </div>
